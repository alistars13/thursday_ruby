...
#methode pour lancer l'ensemble des autres methodes
def lancement_journaliste

  #donne le nombre de journalistes
  ...
  #donne le nombre de journaliste avec un nombre
  def journalist_with_number
    journalist_with_number = []
    journalist_with_number = @journalist.select { |v| v =~ /[1234567890]/ }
    puts "il y a : #{journalist_with_number.length}  journalistes avec une adresse contenant un chiffre."

  end
  #excecution de la methode
  journalist_with_number

  # donne le nombre de journalistes contenant la chaine de charactere "aude"
  def journalist_with_Aude
    journalist_with_Aude = []
    journalist_with_Aude = @journalist.select { |v| v =~ /aude/ ||v =~ /Aude/ }
    puts "il y a : #{journalist_with_Aude.length} adresses qui contienent les lettres du prénom Aude"
  end
  #excecution de la methode
  journalist_with_Aude
  ...
  def journalist_start_with_upper_case
    ...
    journalist_start_with_upper_case = @journalist_without_at.select {|item| item =~ /^(?=.*\b[A-Z]).*/} #prend la premiere valeur du string si elle est en uppercase entre A et Z.
  ...
  def journalist_with_underscore
    ...
    journalist_with_underscore = @journalist.select { |v| v =~ /_/ }
    puts "il y a : #{journalist_with_underscore.length} journalistes avec une adresse contenant un underscore"
  end
  #excecution de la methode
  journalist_with_underscore

  #classement des journalistes en ordre alphabetique
  def journalist_alphabetical_order
  puts @journalist.sort_by { |x| x.downcase }
  end
  #excecution de la methode
  journalist_alphabetical_order
end
#joue la methode globale de journaliste
lancement_journaliste
